...
namespace ssh
{
	...
	public:
		...
		~String() { empty(); }
		// привидение типа
		operator ssh_cws() const { return (ssh_cws)str(); }
		...
		// методы
		ssh_ws* buffer(ssh_u offs = 0) const { return str() + offs; }
		ssh_l length() const { return _str.len; }
		ssh_ws at(ssh_u idx) const { return (idx < (ssh_u)length() ? str()[idx] : L'0'); }
		void set(ssh_u idx, ssh_ws ws) { if(idx < (ssh_u)length()) str()[idx] = ws; }
		...
		const String& lower() { _wcslwr(str()); return *this; }
		const String& upper() { _wcsupr(str()); return *this; }
		const String& reverse() { _wcsrev(str()); return *this; }
		...
		ssh_l find(ssh_ws ws, ssh_l idx = 0) const { auto buf(str()); return (idx < length() ? (wcschr(buf + idx, ws) - buf) : -1); }
		...
		ssh_ws* str() const { return ( _str.len >= SSH_BUFFER_LENGTH ? _str.ptr : (ssh_ws*)_str.str); }
	protected:
