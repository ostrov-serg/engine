...
namespace ssh
{
	...
	public:
		...
		// сохранить
		void save(const String& path, ssh_cws cod);
		...
		template<typename T> void set_attr(HXML h, ssh_cws name, const T& val)
		{
			auto n(h->value->attr(name));
			if(n) n->val = val; else h->value->add_attr(new XmlNode(name, val));
		}
		// получить значение атрибута
		template <typename T> T attr(HXML h, ssh_cws name, const T& def) const
		{
			auto n(h->value->attr(name));
			return (n ? (T)n->val : (T)def);
		}
		// вернуть признак наличия атрибута
		bool is_attr(HXML h, ssh_cws name) const
		{
			return (h->value->attr(name) != nullptr);
		}
		// добавить узел
		template <typename T> HXML add_node(HXML h, ssh_cws name, const T& val)
		{
