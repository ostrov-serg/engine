...
static ssh_l match_ref(ssh_l offset, ssh_cws eptr, ssh_l length, match_data *md, BOOL caseless)
{
	ssh_cws eptr_start = eptr;
	ssh_cws p = md->start_subject + md->offset_vector[offset];
...
//#define return ra) return ra

#define CHECK_PARTIAL()  if (md->partial != 0 && eptr >= md->end_subject && eptr > md->start_used_ptr) { md->hitend = TRUE; if (md->partial > 1) return REGEX_ERROR_PARTIAL; }
#define SCHECK_PARTIAL() if (md->partial != 0 && eptr > md->start_used_ptr) { md->hitend = TRUE; if (md->partial > 1) return REGEX_ERROR_PARTIAL; }

static ssh_l match(ssh_cws eptr, ssh_cws ecode, ssh_cws mstart, ssh_l offset_top, match_data *md, eptrblock *eptrb, ssh_u rdepth)
{
	register ssh_l  rrc;
	register ssh_l  i;
	register ssh_u c;
...
#define fc c
	...
	if(ecode == nullptr)
	{
		if(rdepth == 0) return match((REGEX_PUCHAR)&rdepth, nullptr, nullptr, 0, nullptr, nullptr, 1);
		else
		{
...
TAIL_RECURSE:
	...
	if(md->match_call_count++ >= md->match_limit) return REGEX_ERROR_MATCHLIMIT;
	if(rdepth >= md->match_limit_recursion) return REGEX_ERROR_RECURSIONLIMIT;
	...
	for(;;)
	{
		...
		switch(op)
		{
			case OP_MARK:
				...
				md->mark = nullptr;
				RMATCH(eptr, ecode + OP_lengths[*ecode] + ecode[1], offset_top, md, eptrb, RM55);
				if((rrc == MATCH_MATCH || rrc == MATCH_ACCEPT) && md->mark == nullptr) md->mark = ecode + 2;
			...
			case OP_COMMIT:
				RMATCH(eptr, ecode + OP_lengths[*ecode], offset_top, md, eptrb, RM52);
				if(rrc != MATCH_NOMATCH) return rrc;
				return MATCH_COMMIT;
			case OP_PRUNE:
				RMATCH(eptr, ecode + OP_lengths[*ecode], offset_top, md, eptrb, RM51);
				if(rrc != MATCH_NOMATCH) return rrc;
				return MATCH_PRUNE;
			case OP_PRUNE_ARG:
				md->nomatch_mark = ecode + 2;
				md->mark = nullptr;
				RMATCH(eptr, ecode + OP_lengths[*ecode] + ecode[1], offset_top, md, eptrb, RM56);
				if((rrc == MATCH_MATCH || rrc == MATCH_ACCEPT) && md->mark == nullptr) md->mark = ecode + 2;
				if(rrc != MATCH_NOMATCH) return rrc;
				return MATCH_PRUNE;
			case OP_SKIP:
				RMATCH(eptr, ecode + OP_lengths[*ecode], offset_top, md, eptrb, RM53);
				if(rrc != MATCH_NOMATCH) return rrc;
				md->start_match_ptr = eptr;
				return MATCH_SKIP;
			case OP_SKIP_ARG:
				...
				{
					ecode += OP_lengths[*ecode] + ecode[1];
					break;
				}
				RMATCH(eptr, ecode + OP_lengths[*ecode] + ecode[1], offset_top, md, eptrb, RM57);
				if(rrc != MATCH_NOMATCH) return rrc;
				md->start_match_ptr = ecode + 2;
				return MATCH_SKIP_ARG;
			case OP_THEN:
				RMATCH(eptr, ecode + OP_lengths[*ecode], offset_top, md, eptrb, RM54);
				if(rrc != MATCH_NOMATCH) return rrc;
			...
			case OP_THEN_ARG:
				md->nomatch_mark = ecode + 2;
				md->mark = nullptr;
				RMATCH(eptr, ecode + OP_lengths[*ecode] + ecode[1], offset_top, md, eptrb, RM58);
				if((rrc == MATCH_MATCH || rrc == MATCH_ACCEPT) && md->mark == nullptr) md->mark = ecode + 2;
				if(rrc != MATCH_NOMATCH) return rrc;
				md->start_match_ptr = ecode;
				return MATCH_THEN;
			case OP_ONCE_NC:
				...
				if(*ecode != OP_ONCE_NC && *ecode != OP_ALT) return MATCH_NOMATCH;
			...
			case OP_SCBRA:
				...
				if(offset < md->offset_max)
				{
					...
					for(;;)
					{
						...
						RMATCH(eptr, ecode + OP_lengths[*ecode], offset_top, md, eptrb, RM1);
			...
			case OP_SBRA:
				for(;;)
				{
					...
					{
						ecode += OP_lengths[*ecode];
						goto TAIL_RECURSE;
					}
					save_mark = md->mark;
					save_capture_last = md->capture_last;
					RMATCH(eptr, ecode + OP_lengths[*ecode], offset_top, md, eptrb, RM2);
					if(rrc == MATCH_THEN)
					{
						next = ecode + GET(ecode, 1);
						if(md->start_match_ptr < next && (*ecode == OP_ALT || *next == OP_ALT))
...
POSSESSIVE_CAPTURE:
				...
				{
					md->offset_vector[md->offset_end - number] = (ssh_l)(eptr - md->start_subject);
					if(op >= OP_SBRA) md->match_function_type = MATCH_CBEGROUP; RMATCH(eptr, ecode + OP_lengths[*ecode], offset_top, md, eptrb, RM63);
...
POSSESSIVE_NON_CAPTURE:
				...
				{
					if(op >= OP_SBRA) md->match_function_type = MATCH_CBEGROUP;
					RMATCH(eptr, ecode + OP_lengths[*ecode], offset_top, md, eptrb, RM48);
					if(rrc == MATCH_KETRPOS)
					{
						offset_top = md->end_offset_top;
						ecode = md->start_code + code_offset;
			...
			case OP_SCOND:
				...
				if(*ecode == OP_CALLOUT)
				{
					ecode += OP_lengths[OP_CALLOUT];
					codelink -= OP_lengths[OP_CALLOUT];
				}
				condition = FALSE;
				switch(condcode = *ecode)
				{
					case OP_RREF:
						if(md->recursive != nullptr)
						{
					...
					default:
						...
						RMATCH(eptr, ecode, offset_top, md, nullptr, RM3);
						if(rrc == MATCH_MATCH)
						{
							...
							ecode += 1 + LINK_SIZE - OP_lengths[condcode];
						}
				...
				ecode += condition ? OP_lengths[condcode] : codelink;
			...
			case OP_ASSERT_ACCEPT:
				if(eptr == mstart && op != OP_ASSERT_ACCEPT && md->recursive == nullptr && (md->notempty || (md->notempty_atstart && mstart == md->start_subject + md->start_offset)))
				   return MATCH_NOMATCH;
			...
			case OP_ASSERTBACK:
				...
				do
				{
					RMATCH(eptr, ecode + 1 + LINK_SIZE, offset_top, md, nullptr, RM4);
				...
				if(condassert) return MATCH_MATCH;
				do ecode += GET(ecode, 1); while(*ecode == OP_ALT);
				ecode += 1 + LINK_SIZE;
				offset_top = md->end_offset_top;
				continue;
			...
			case OP_ASSERTBACK_NOT:
				...
				do
				{
					RMATCH(eptr, ecode + 1 + LINK_SIZE, offset_top, md, nullptr, RM5);
					md->mark = save_mark;
...
NEG_ASSERT_TRUE:
			...
			case OP_RECURSE:
			{
				...
				for(ri = md->recursive; ri != nullptr; ri = ri->prevrec)
					if(recno == ri->group_num && eptr == ri->subject_position) return REGEX_ERROR_RECURSELOOP;
				...
				else
				{
					...
					if(new_recursive.offset_save == nullptr) return REGEX_ERROR_NOMEMORY;
				}
				...
				do
				{
					...
					RMATCH(eptr, callpat + OP_lengths[*callpat], offset_top, md, eptrb, RM6);
...
RECURSION_MATCHED:
			...
			case OP_KETRPOS:
				...
				if(*prev == OP_CBRA || *prev == OP_SCBRA || *prev == OP_CBRAPOS || *prev == OP_SCBRAPOS)
				{
					...
					offset = number << 1;
					if(md->recursive != nullptr && md->recursive->group_num == number)
					{
						md->end_match_ptr = eptr;
						md->start_match_ptr = mstart;
						return MATCH_MATCH;
			...
			case OP_DOLLM:
				if(eptr < md->end_subject)
				{
					if(!IS_NEWLINE(eptr))
					{
						...
						{
							md->hitend = TRUE;
							if(md->partial > 1) return REGEX_ERROR_PARTIAL;
						}
						return MATCH_NOMATCH;
					}
				}
...
ASSERT_NL_OR_EOS :
			...
			break;
			case OP_ANY:
				if(IS_NEWLINE(eptr)) return MATCH_NOMATCH;
				if(md->partial != 0 && eptr + 1 >= md->end_subject && NLBLOCK->nltype == NLTYPE_FIXED && NLBLOCK->nllen == 2 && UCHAR21TEST(eptr) == NLBLOCK->nl[0])
				{
					md->hitend = TRUE;
					if(md->partial > 1) return REGEX_ERROR_PARTIAL;
			...
			case OP_ANYNL:
				...
				switch(c)
				{
					...
					case 0x2028:
					case 0x2029:
						if(md->bsr_anycrlf) return MATCH_NOMATCH;
						break;
				}
				ecode++;
				break;
			...
			case OP_NOT_HSPACE:
				...
				switch(c)
				{
					HSPACE_CASES: return MATCH_NOMATCH;
					default: break;
				}
				ecode++;
				break;
			...
			case OP_NOT_VSPACE:
				...
				switch(c)
				{
					VSPACE_CASES: return MATCH_NOMATCH;
					default: break;
				}
				ecode++;
				break;
...
REF_REPEAT:
				...
				if(minimize)
				{
					for(fi = min;; fi++)
					{
						...
						if(rrc != MATCH_NOMATCH) return rrc;
						if(fi >= max) return MATCH_NOMATCH;
						if((slength = match_ref(offset, eptr, length, md, caseless)) < 0)
						{
							if(slength == -2) eptr = md->end_subject;
							CHECK_PARTIAL();
...
#define BYTE_MAP ((ssh_b *)data)
				...
				for(i = 1; i <= min; i++)
				{
					...
					if(c > 255)
					{
						...
						if((BYTE_MAP[c / 8] & (1 << (c & 7))) == 0) return MATCH_NOMATCH;
				}
				if(min == max) continue;
				if(minimize)
				{
...
#undef BYTE_MAP
			...
			case OP_XCLASS:
			{
				...
				for(i = 1; i <= min; i++)
				{
					...
					GETCHARINCTEST(c, eptr);
					if(!xclass(c, data, utf)) return MATCH_NOMATCH;
				}
				if(min == max) continue;
				if(minimize)
				{
			...
			case OP_CHAR:
				...
				if(ecode[1] != *eptr++) return MATCH_NOMATCH;
				ecode += 2;
				break;
			case OP_CHARI:
				if(eptr >= md->end_subject)
				...
				if(TABLE_GET(ecode[1], md->lcc, ecode[1]) != TABLE_GET(*eptr, md->lcc, *eptr)) return MATCH_NOMATCH;
				eptr++;
				ecode += 2;
				break;
			case OP_EXACT:
...
REPEATCHAR:
				...
				if(op >= OP_STARI)
				{
					...
					for(i = 1; i <= min; i++)
					{
						...
						cc = UCHAR21TEST(eptr);
						if(fc != cc && foc != cc) return MATCH_NOMATCH;
						eptr++;
					}
					if(min == max) continue;
					if(minimize)
				...
				else
				{
					for(i = 1; i <= min; i++)
					{
						...
						if(fc != UCHAR21INCTEST(eptr)) return MATCH_NOMATCH;
					}
			...
			case OP_NOTI:
				...
				}
				{
					...
					c = *eptr++;
					if(ch == c || (op == OP_NOTI && TABLE_GET(ch, md->fcc, ch) == c)) return MATCH_NOMATCH;
					ecode += 2;
				}
				break;
			case OP_NOTEXACT:
...
REPEATNOTCHAR:
				...
				if(op >= OP_NOTSTARI)
				{
					...
					for(i = 1; i <= min; i++)
					{
						...
						if(fc == *eptr || foc == *eptr) return MATCH_NOMATCH;
						eptr++;
					}
					if(min == max) continue;
				...
				else
				{
					for(i = 1; i <= min; i++)
					{
						...
						if(fc == *eptr++) return MATCH_NOMATCH;
					}
					if(min == max) continue;
					if(minimize)
					{
						...
						switch(ctype)
						{
								case OP_ANY:
									for(i = 1; i <= min; i++)
									{
										...
										if(IS_NEWLINE(eptr)) return MATCH_NOMATCH;
										if(md->partial != 0 && eptr + 1 >= md->end_subject && NLBLOCK->nltype == NLTYPE_FIXED && NLBLOCK->nllen == 2 && *eptr == NLBLOCK->nl[0])
										{
											md->hitend = TRUE;
											if(md->partial > 1) return REGEX_ERROR_PARTIAL;
										}
				...
				{
					{
							for(fi = min;; fi++)
							{
								...
								{
									case OP_ANY:
										if(md->partial != 0 && eptr >= md->end_subject && NLBLOCK->nltype == NLTYPE_FIXED && NLBLOCK->nllen == 2 && c == NLBLOCK->nl[0])
										{
											md->hitend = TRUE;
											if(md->partial > 1) return REGEX_ERROR_PARTIAL;
										}
									...
									case OP_NOT_DIGIT:
										if(MAX_255(c) && (md->ctypes[c] & ctype_digit) != 0) return MATCH_NOMATCH;
										break;
									...
									case OP_NOT_WHITESPACE:
										if(MAX_255(c) && (md->ctypes[c] & ctype_space) != 0) return MATCH_NOMATCH;
										break;
									...
									case OP_NOT_WORDCHAR:
										if(MAX_255(c) && (md->ctypes[c] & ctype_word) != 0) return MATCH_NOMATCH;
										break;
...
ENDLOOP03:
			...
			default:
				return REGEX_ERROR_UNKNOWN_OPCODE;
		}
	}
}
...
__declspec(dllexport) ssh_l regex16_exec(const regex16 *argument_re, ssh_cws subject, ssh_l length, ssh_l start_offset, ssh_l options, ssh_l* offsets, ssh_l offsetcount)
{
	...
	const ssh_b *tables;
	const ssh_b *start_bits = nullptr;
	ssh_cws start_match = (ssh_cws)subject + start_offset;
	ssh_cws end_subject;
	ssh_cws start_partial = nullptr;
	ssh_cws match_partial = nullptr;
	ssh_cws req_char_ptr = start_match - 1;
	const regex_study_data *study;
	const REAL_PCRE *re = (const REAL_PCRE *)argument_re;
	if((options & ~PUBLIC_EXEC_OPTIONS) != 0) return REGEX_ERROR_BADOPTION;
	if(re == nullptr || subject == nullptr || (offsets == nullptr && offsetcount > 0)) return REGEX_ERROR_NULL;
	...
	if(tables == nullptr) tables = default_tables;
	...
	md->hitend = FALSE;
	md->mark = md->nomatch_mark = nullptr;
	md->recursive = nullptr;
	...
	if(re->top_backref > 0 && re->top_backref >= ocount / 3)
	{
		...
		if(md->offset_vector == nullptr) return REGEX_ERROR_NOMEMORY;
	...
	if(!anchored)
	{
		if((re->flags & REGEX_FIRSTSET) != 0)
		{
			...
			if(!startline && study != nullptr && (study->flags & REGEX_STUDY_MAPPED) != 0)
	...
	for(;;)
	{
		...
		{
			if(study != nullptr && (study->flags & REGEX_STUDY_MINLEN) != 0 && (ssh_u)(end_subject - start_match) < study->minlength)
			{
				rc = MATCH_NOMATCH;
				break;
			}
		...
		md->end_offset_top = 0;
		md->skip_arg_count = 0;
		rc = match(start_match, md->start_code, start_match, 2, md, nullptr, 0);
		if(md->hitend && start_partial == nullptr)
		{
...
ENDLOOP:
	if(rc == MATCH_MATCH || rc == MATCH_ACCEPT)
	{
		...
		if(md->end_offset_top / 2 <= re->top_bracket && offsets != nullptr)
		{
			ssh_l* iptr, *iend;
