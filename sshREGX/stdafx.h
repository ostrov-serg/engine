...
#define IS_NEWLINE(p) ((NLBLOCK->nltype != NLTYPE_FIXED) ? ((p) < NLBLOCK->PSEND && is_newline((p), NLBLOCK->nltype, NLBLOCK->PSEND, &(NLBLOCK->nllen), utf)) : ((p) <= NLBLOCK->PSEND - NLBLOCK->nllen && UCHAR21TEST(p) == NLBLOCK->nl[0] && (NLBLOCK->nllen == 1 || UCHAR21TEST(p+1) == NLBLOCK->nl[1])))
#define WAS_NEWLINE(p) ((NLBLOCK->nltype != NLTYPE_FIXED) ? ((p) > NLBLOCK->PSSTART && was_newline((p), NLBLOCK->nltype, NLBLOCK->PSSTART, &(NLBLOCK->nllen), utf)) : ((p) >= NLBLOCK->PSSTART + NLBLOCK->nllen && UCHAR21TEST(p - NLBLOCK->nllen) == NLBLOCK->nl[0] && (NLBLOCK->nllen == 1 || UCHAR21TEST(p - NLBLOCK->nllen + 1) == NLBLOCK->nl[1])))
...
typedef struct compile_data
{
	...
	const ssh_b *cbits;
	const ssh_b *ctypes;
	ssh_cws start_workspace;
	ssh_cws start_code;
	ssh_cws start_pattern;
	ssh_cws end_pattern;
	ssh_ws* hwm;
...
struct match_data
{
	...
	BOOL	hasthen;
	ssh_cws	start_code;
	ssh_cws start_subject;
	ssh_cws end_subject;
	ssh_cws start_match_ptr;
	ssh_cws end_match_ptr;
	ssh_cws start_used_ptr;
	ssh_l	partial;
...
extern const ssh_b		OP_lengths[];
extern const ssh_b		default_tables[];
extern const ssh_u		hspace_list[];
extern const ssh_u		vspace_list[];
extern ssh_l			strncmp_uc_c8(ssh_cws, ssh_ccs, ssh_u num);

#define STRCMP_UC_UC(str1, str2) wcscmp(str1, str2)
#define STRNCMP_UC_UC(str1, str2, num) wcsncmp(str1, str2, num)
#define STRNCMP_UC_C8(str1, str2, num) strncmp_uc_c8((str1), (str2), (num))
#define STRCMP_UC_UC_TEST(str1, str2) STRCMP_UC_UC(str1, str2)

extern ssh_cws			find_bracket(ssh_cws, BOOL, ssh_l);
extern BOOL				is_newline(ssh_cws, ssh_l, ssh_cws, ssh_l*, BOOL);
extern ssh_u			ord2utf(ssh_u, ssh_ws*);
extern ssh_l			valid_utf(ssh_cws, ssh_l, ssh_l*);
extern BOOL				was_newline(ssh_cws, ssh_l, ssh_cws, ssh_l*, BOOL);
extern BOOL				xclass(ssh_u, ssh_cws, BOOL);
...
extern const ssh_u ucd_caseless_sets[];
extern const ucd_record  ucd_records[];
extern const ssh_b  ucd_stage1[];
extern const ssh_ws ucd_stage2[];
extern const ssh_u ucp_gentype[];
extern const ssh_u ucp_gbtable[];
